      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {Object.entries(suggestion).map(([k, v]) => (
          <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontFamily: "var(--font-mono)" }}>
            ...
            <span style={{ color: "var(--color-ink-100)", fontWeight: 600 }}>
...
export function OperationalMode() {
  const [target, setTarget] = useState("Power Conversion Efficiency (PCE)");
  const [variables, setVariables] = useState<OperationalVariable[]>([
    { name: "x_3MTPAI", min: 0.0, max: 1.0, unit: "Ratio" },
    { name: "x_PDAI2", min: 0.0, max: 1.0, unit: "Ratio" },
    { name: "x_EDAI2", min: 0.0, max: 1.0, unit: "Ratio" },
    { name: "x_PipDI", min: 0.0, max: 1.0, unit: "Ratio" }
  ]);
  const [history, setHistory] = useState<OperationalObservation[]>([
    {
      config: { x_3MTPAI: 0.20, x_PDAI2: 0.55, x_EDAI2: 0.10, x_PipDI: 0.15 },
      score: 25.07
    },
    {
      config: { x_3MTPAI: 0.35, x_PDAI2: 0.20, x_EDAI2: 0.35, x_PipDI: 0.15 },
      score: 25.23
    },
    {
      config: { x_3MTPAI: 0.45, x_PDAI2: 0.15, x_EDAI2: 0.35, x_PipDI: 0.05 },
      score: 26.00
    }
  ]);
