...
# Ground-truth global best yield per dataset (verified from test.csv maxima).
...
def assert_seed_completeness(
    ...
    """Verify every (composition, dataset) has exactly the expected seeds.

    Raises ValueError if any composition is missing seeds on any dataset,
    which would make cross-composition comparison unfair.
    """
    expected = set(expected_seeds)
    grouped: dict[tuple[str, str], set[int]] = {}
    for r in results:
        key = (r["composition"], r["dataset"])
        grouped.setdefault(key, set()).add(r["seed"])

    missing: list[str] = []
    for (comp, ds), seeds in sorted(grouped.items()):
        absent = expected - seeds
        if absent:
            missing.append(f"{comp}/{ds} missing seeds {sorted(absent)}")
    if missing:
        raise ValueError(
...
def _normalize_metric(metric: str, mean: float, ci95: float, dataset: str) -> float:
    ...
    """
    gbest = GLOBAL_BEST.get(dataset, 100.0)
    if metric == "t95":
