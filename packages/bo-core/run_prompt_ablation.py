...
Results are saved to packages/bo-core/tmp_results/prompt_ablation/
"""
...
def patch_variant_b():
    ...
    from bo_core.optimization import knowledge as k
...
def patch_variant_c():
    ...
    def patched_eval(self, candidate, system_prompt, feature_cols, gp_mean=None, gp_std=None):
        ...
        from bo_core.llm_client import LlmCallResult
    ...
    from bo_core.optimization import optimizer as opt
    ...
    def patched_suggest(self, top_k=20, n_candidates=5, acquisition="ucb",
        ...
        if use_llm and use_logprobs and self.knowledge_engine._client.is_configured() and not top_candidates.empty:
            ...
            from bo_core.optimization.knowledge import SuggestionResult
            suggestions = [
...
def run_variant(variant_name: str, patch_fn, output_dir: Path):
    ...
    from bo_core.optimization import knowledge, optimizer
