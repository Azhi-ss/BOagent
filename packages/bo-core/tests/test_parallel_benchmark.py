...
class TestParallelBenchmark(unittest.TestCase):
    ...
    @patch("bo_core.benchmark.comparison.DATA_LOADERS", dict())
    @patch("bo_core.benchmark.comparison.BOStepEngine")
    def test_parallel_execution_events(self, mock_bo_step):
        from bo_core.benchmark.comparison import DATA_LOADERS
        # Setup mocks
