...
from bo_core.benchmark.data_loader import BAND_ALIGNMENT_FEATURES, TARGET_COL
...
class TestBenchmarkRunner:
    ...
    def test_run_produces_result_dict(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(
                "bo_core.benchmark.runner._resolve_pvk_root",
                lambda: Path("/fake/pvk"),
            )

            with patch(
                "bo_core.benchmark.runner.BayesianOptimizer",
                autospec=True,
            ) as mock_opt_class:
                mock_opt = MagicMock()
                # Mock suggest result
